...
* _Ресурс_: необязательное поле; в случае использования jabber-аккаунта в различных местах или с помощью различных устройств, можно указать в этом поле к примеру `work`, `home`, `android` и т.д.<br/>
...
* _Безопасность соединения_: проверяем, что значение поля установлено в `Требовать шифрование`; эта опция очень важна, т.к. гарантирует, что связь с сервером будет осуществляться только по защищенному каналу с помощью SSL/TLS
...
После нажатия на _"Присоединиться"_ вы попадете в чат. Сообщите остальным собеседникам название комнаты, сервер и пароль, чтобы они тоже присоединились к беседе.
...
_**Внимание!**<br/>
Переписка в чате плагином OTR не шифруется! Чтобы обезопасить свое общение в чате, убедитесь, что каждый собеседник чата при настройке своего аккаунта установил значение поля "Безопасность соединения" в `Требовать шифрование`, а также дополнительно можно воспользоваться [анонимизацией через TOR](#Анонимизация-через-tor)._
