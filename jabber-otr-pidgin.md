...
* _Безопасность соединения_: проверяем, что значение поля установлено в `Требовать шифрование`; ![<Важно!>](/images/a-star.png) _**эта опция очень важна, т.к. гарантирует, что связь с сервером будет осуществляться только по защищенному каналу с помощью SSL/TLS, а также то, что пароль от вашего аккаунта не будет перехвачен**_
